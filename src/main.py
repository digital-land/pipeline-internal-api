...
async def validation_exception_handler(request: Request, exc: ValidationError):
    ...
    for error in errors:
        loc = ".".join(str(x) for x in error["loc"])  # Location of the error (e.g., "query.dataset")
...
def issues(http_response: Response, params: IssuesParams = Depends()):
    ...
    return Response(content=json.dumps(paginated_result.data), media_type="application/json", 
